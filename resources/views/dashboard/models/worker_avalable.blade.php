<div class="modal fase " id="all_worker_avalable" data-backdrop="static" data-keyboard="false" tabindex="-1"
...
<div class="modal-dialog modal-lg">
    <div class="modal-content">
                ...
                <table class="table table-striped table-bordered zero-configuration">
                    ...
                    <tbody>
                      @if(auth()->user()->hasRole('Admin'))
                        ...
                        @php
                            $companies = App\Worker::where('status',1)->orderBy('id', 'DESC')->take(10)->get();
                        @endphp
                        @else
                        @php
                            $companies = App\Worker::where('status',1)->where('company_id',auth()->user()->company->id)->orderBy('id', 'DESC')->take(10)->get();
                        @endphp
                        @endif
        ...
        <div class="modal-footer">
            <a href="{{ route('worker.index',['status'=>1]) }}" class="btn btn-info">
                @lang('Show All')
            </a>
        </div>
